#!/usr/bin/env bash
# Verify the session suite without owning the TTY or the session.
#
# Two things make this possible:
#
#  1. **A redirected runtime dir.** The daemon is single-instance (an flock
#     under $XDG_RUNTIME_DIR/kallos), so a kallosd under test cannot come up
#     beside the one running your desktop. Give it an XDG_RUNTIME_DIR of its
#     own and it can, without either noticing the other.
#
...
#    the machine. Enumeration is exercised; the write paths are not touched.
#
...
KD="$root/kallosd/target/$cfg/kallosd"
CTL="$root/kallosctl/target/$cfg/kallosctl"
...
# The shared labels, plus the tallies this script keeps. `pass`/`fail`/`skip`
# in lib/out.sh print; these wrap them to count as well, so the summary at the
# bottom cannot drift from the lines above it.
n_pass=0 n_fail=0 n_skip=0
yes() { pass "$*"; n_pass=$((n_pass + 1)); }
no()  { fail "$*"; n_fail=$((n_fail + 1)); }
skp() { skip "$*"; n_skip=$((n_skip + 1)); }

# ---- preflight ------------------------------------------------------------
for b in "$KD" "$CTL" "$HAJIME" "$PHYLAX"; do
	[ -x "$b" ] || die "missing $b — run scripts/build.sh $cfg"
done
...
if [ "${FORCE:-0}" != 1 ] && pgrep -x kosmos >/dev/null; then
	err "a compositor is running — this script claims session-bus names and"
	die "pushes overlay verbs. Stop the session, or FORCE=1 to override."
fi

# sockaddr_un.sun_path is 108 bytes: a long scratch path fails at listen(),
# which is a confusing way to learn this.
sock="$V/rt/kallos/kdaemon.sock"
[ ${#sock} -lt 100 ] || die "VDIR too long: $sock would not fit in sun_path"

rm -rf "$V"
mkdir -p "$V/rt" "$V/home/.config/kallos"
chmod 700 "$V/rt"
...
# the lists have something realistic to work from.
...
cleanup() { pkill -x kallosd 2>/dev/null || true
            pkill -x kosmos 2>/dev/null || true; pkill -x hajime 2>/dev/null || true
            pkill -x phylax 2>/dev/null || true; }
trap cleanup EXIT

hdr "verify — the $cfg build"
note "scratch dir $V"

# ---- 1. the daemon answers ------------------------------------------------
# There is one daemon and nothing to diff it against, so these checks say what
# they can say on their own: it listens, the lists render through the real
# consumer, and no verb takes it down.
sec "the daemon answers"
XDG_RUNTIME_DIR="$V/rt" "$KD" --verbose
sleep 2   # let it finish connecting to PulseAudio and priming its caches

[ -S "$V/rt/kallos/kdaemon.sock" ] && yes "kallosd is listening" || no "kallosd never listened"

# Every list, through the real consumer. Not compared against anything — just
# that hajime can ask and the daemon can answer without either hanging.
for m in --lists --apps; do
	if XDG_RUNTIME_DIR="$V/rt" timeout 10 "$HAJIME" $m >"$V/lists$m" 2>&1; then
		yes "hajime $m rendered $(wc -l <"$V/lists$m") line(s)"
	else
		no "hajime $m failed or hung:"; sed 's/^/       /' "$V/lists$m" | head -10
	fi
done

# The command surface. Deliberately no media/power/display/overlay: the session
# bus is not redirected and those reach the real desktop. Several of these are
# MEANT to fail (`bt confirm` with nothing pending, `run something` with no
# such binary), so the exit status is not the check — surviving them is. A verb
# that panics the daemon takes the socket with it, which the probe after the
# sweep is what catches.
cases=(
...
for c in "${cases[@]}"; do
	...
	XDG_RUNTIME_DIR="$V/rt" timeout 10 "$CTL" $c >/dev/null 2>&1 || true
done
if XDG_RUNTIME_DIR="$V/rt" timeout 10 "$CTL" config reload >/dev/null 2>&1; then
	yes "the daemon survived all ${#cases[@]} commands"
else
	no "the daemon stopped answering during the command sweep"
fi

# ---- 2. the poller parks --------------------------------------------------
sec "the poller parks at 0% CPU with no subscribers"
pid=$(pgrep -x kallosd | head -1)
ticks() { awk '{print $14 + $15}' "/proc/$1/stat"; }
t0=$(ticks "$pid"); sleep 10; t1=$(ticks "$pid")
if [ $((t1 - t0)) -le 1 ]; then
	yes "kallosd used $((t1 - t0)) tick(s) over 10s idle"
else
	no "kallosd used $((t1 - t0)) ticks over 10s idle — the poller is not parking"
fi

pkill -x kallosd || true; sleep 1

# ---- 3. the session root (M10) --------------------------------------------
sec "session root: kallosd --wm --overlay on a headless compositor"
if [ ! -x "$KOSMOS" ]; then
	skp "no kosmos built — session test skipped"
else
	rm -f "$V/startup-marker"
	log="$V/session.log"
	XDG_RUNTIME_DIR="$V/rt" WLR_BACKENDS=headless WLR_HEADLESS_OUTPUTS=2 \
		nohup "$KD" --verbose --wm "$KOSMOS" --overlay "$HAJIME" >"$log" 2>&1 &
	...
	grep -q 'session display registered' "$log" \
		&& yes "compositor registered its WAYLAND_DISPLAY over SESSION" \
		|| no "no SESSION register — see $log"
	pgrep -x hajime >/dev/null \
		&& yes "overlay spawned on the compositor's display" \
		|| no "overlay never spawned"
	grep -q "layer surface 'hajime'" "$log" \
		&& yes "overlay mapped its OVERLAY layer surface" \
		|| no "overlay surface never mapped"
	[ -e "$V/startup-marker" ] \
		&& yes "startup commands ran on the SESSION register" \
		|| no "startup commands did not run"

	# Invariant: spawned children get a reset signal mask. The daemon blocks
	# SIGCHLD for its signalfd, and that mask survives exec — a real bug once,
	# where a terminal spawned from the session had no working Ctrl+C.
	hp=$(pgrep -x hajime | head -1)
	blk=$(awk '/^SigBlk/{print $2}' "/proc/$hp/status" 2>/dev/null || echo missing)
	[ "$blk" = "0000000000000000" ] \
		&& yes "spawned child has an empty signal mask" \
	...
	XDG_RUNTIME_DIR="$V/rt" "$CTL" overlay show >/dev/null; sleep 2
	XDG_RUNTIME_DIR="$V/rt" "$CTL" overlay hide >/dev/null; sleep 1
	XDG_RUNTIME_DIR="$V/rt" "$CTL" overlay toggle >/dev/null; sleep 1
	[ "$(grep -c 'overlay verb=' "$log")" -ge 3 ] \
		&& yes "overlay show/hide/toggle reached the resident overlay" \
	...
	rm -f "$V/startup-marker"
	XDG_RUNTIME_DIR="$V/rt" "$CTL" config reload >/dev/null; sleep 1
	[ ! -e "$V/startup-marker" ] \
		&& yes "config reload did not re-run the startup commands" \
	...
	if [ ! -x "$DC" ]; then
		...
		"$CTL" display HEADLESS-1 position 2000 100 >/dev/null
		if "$DC" --expect "HEADLESS-1=2000,100,1280x720" >/dev/null; then
			yes "an explicit position lands where asked"
		else
		...
		if "$DC" --expect "HEADLESS-2=$h2,1280x720" >/dev/null; then
			yes "the other output did not drift"
		else
			no "moving one output moved the other (the pin-before-move rule)"
		fi

		"$CTL" display HEADLESS-1 transform 90 >/dev/null
		if "$DC" --expect "HEADLESS-1=2000,100,720x1280" >/dev/null; then
			yes "a 90 transform swaps the logical size"
		else
			no "transform did not swap logical width/height"
		fi
		"$CTL" display HEADLESS-1 transform normal >/dev/null
		...
		"$CTL" display HEADLESS-1 disable >/dev/null
		sleep 1
		"$CTL" display HEADLESS-1 enable >/dev/null
		if "$DC" --expect "HEADLESS-1=2000,100,1280x720" --expect "HEADLESS-2=$h2,1280x720" >/dev/null; then
			yes "disable then enable restored both positions"
		else
		...
		if grep -q 'position 2000 100' "$dpy_cfg" 2>/dev/null; then
			yes "the arrangement persisted itself to the displays config"
		else
		...
		"$CTL" display HEADLESS-1 position 2000 500 >/dev/null
		"$CTL" display HEADLESS-2 position 2000 1220 >/dev/null
		sleep 1
		if "$DC" --print | awk '{split($2,p,","); if (p[1]==0 && p[2]==0) found=1}
		                        END {exit !found}'; then
			yes "the arrangement stays anchored to the layout origin"
		else
	...
	if [ ! -x "$PTYPE" ]; then
		...
		WAYLAND_DISPLAY=$(sed -n 's/.*WAYLAND_DISPLAY=\([a-z0-9-]*\).*/\1/p' "$log" | head -1)
		XDG_RUNTIME_DIR="$V/rt" "$CTL" power lock >/dev/null; sleep 3
		grep -q 'kallosd\] session locked' "$log" \
			&& yes "power lock locked the session (compositor relay seen)" \
			|| no "no session-locked relay after power lock"
		XDG_RUNTIME_DIR="$V/rt" WAYLAND_DISPLAY="$WAYLAND_DISPLAY" "$PTYPE" 'wrong\n' >/dev/null; sleep 2
		pgrep -x phylax >/dev/null && ! grep -q 'session unlocked' "$log" \
			&& yes "a wrong password left it locked" \
		...
		grep -q 'kallosd\] session unlocked' "$log" && ! pgrep -x phylax >/dev/null \
			&& yes "the right password unlocked and the locker exited" \
		...
		pgrep -x phylax >/dev/null && grep -q 'respawning locker' "$log" \
			&& yes "a crashed locker was respawned with the session still locked" \
...
hdr "verify — $n_pass passed, $n_fail failed, $n_skip skipped"
[ "$n_fail" -eq 0 ]
