#!/usr/bin/env bash
# Build + install + run a primary-TTY Kallos session with verbose kwm logging.
...
# build.sh builds all four binaries — the three cargo crates plus kwm through
...
# --wm points at the freshly installed binary so the flags reach kwm.
...
KWM_PLANES_DUMP=1 \
	kallosd --wm "$PREFIX/bin/kwm --verbose" \
...
#   coredumpctl list kwm        # should show NO new crash
# Note kwm's SIGTERM teardown still aborts on a wlroots wlr_session_lock_v1
